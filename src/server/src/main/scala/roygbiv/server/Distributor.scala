...
class Distributor extends Actor {
  ...
  def receive = {
    case s: Scene ⇒
      scene = Some(s)
      println("Scene")
      context.actorFor(context.system.settings.config.getString("akka.raytracing.aggregator.address")) ! s
    case c: ClientRegistration ⇒
      println("clinet registration")
      clients = c +: clients
      if (scene.get == None) println("None")
